...
const TRIP_FIELDS = "id,title,description,rating,review_count,key_highlight,estimate_price,total_time,url_image,user_id,created_at,is_post";
...
export async function listTrips(req, res) {
  ...
  if (!["rating","review_count","estimate_price","total_time","title","id"].includes(column)) column = "rating";
  direction = direction === "asc" ? "ASC" : "DESC";

  try {
    const like = `%${search}%`;
    const rows = await all(
      `SELECT t.${TRIP_FIELDS.split(",").join(",t.")},
       CASE WHEN favorite.trip_id IS NULL THEN 0 ELSE 1 END AS is_favorite
       FROM trips t LEFT JOIN user_favorite_trips favorite
      ...
      [req.user?.id || -1,search,like,like,like,minRating,maxPrice,limit,offset]
    );
...
export async function createTrip(req, res) {
  ...
  try {
    const trip = await transaction(async queries => {
      ...
      return queries.get(`SELECT ${TRIP_FIELDS} FROM trips WHERE id=?`, [inserted.lastID]);
    });
...
export async function updateTrip(req, res) {
  ...
  try {
    const trip = await transaction(async queries => {
      ...
      return queries.get(`SELECT ${TRIP_FIELDS} FROM trips WHERE id=?`, [owned.id]);
    });
    res.json(trip);
  } catch (error) {
...
export async function getUserTrip(req, res) {
  try {
    res.json(await all(`SELECT ${TRIP_FIELDS} FROM trips WHERE user_id=? ORDER BY created_at DESC,id DESC`, [req.user.id]));
  } catch (error) {
...
async function setPublished(req, res, value) {
  try {
    const result = await run("UPDATE trips SET is_post=? WHERE id=? AND user_id=?", [value,req.params.id,req.user.id]);
    if (!result.changes) return res.status(404).json({ error: "Trip not found or not owned by user" });
    res.json({ message: value ? "Trip published" : "Trip unpublished", trip_id:Number(req.params.id), is_post:value });
  } catch (error) {
    console.error("Could not update publication status:", error.message);
    res.status(500).json({ error: "Could not update publication status" });
  }
}
